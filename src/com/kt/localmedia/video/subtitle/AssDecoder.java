...
public class AssDecoder extends SubtitleDecoder {
    private final String mTag = "AssDecoder";
	...
	public Map<String, List<SubtitleContent>> decodeSubtitle(String subTitlePath,String encode)
		...
		LogUtil.Log(LogUtil.DEBUG_SUBTITLE,mTag,"decodeSubtitle<type ass> path="+subTitlePath);
		...
		while((subTitleLine = br.readLine()) != null && SubContentUtil.stopDecodeFlag == false){
			...
			if(subTitleLine.toUpperCase().startsWith("DIALOGUE:")){
				...
				AssTokenizer assTokenizer = new AssTokenizer(new InputStreamReader(new ByteArrayInputStream(markedStr.getBytes(SubContentUtil.encodeType)), SubContentUtil.encodeType));
				...
				if(markedArray != null && markedArray.length != 0){
					...
					if(subTitleStypeMap != null && subTitleStypeMap.containsKey(lanClassStr)){
						content.setSubtitleLine("<font size=\"" + subTitleStypeMap.get(lanClassStr).getFontSize() + "\" color=\"" + subTitleStypeMap.get(lanClassStr).getFontColor() + "\">" + subTitleSb.toString() + "</font>");
					}else{
					...
					if(subTitleMap.get(lanClassStr) == null){
						subTitleMap.put(lanClassStr, new ArrayList<SubtitleContent>());
					}
					...
					if(!subTimeMap.containsKey(content.getSubtitleStartTime())){
						...
						if(content.getmLanguageClass().equals(subTimeMap.get(content.getSubtitleStartTime()).getmLanguageClass())){
							...
							if(subTitleStypeMap != null && subTitleStypeMap.containsKey(lanClassStr)){
								content.setSubtitleLine("<font size=\"" + subTitleStypeMap.get(lanClassStr).getFontSize() + "\" color=\"" + subTitleStypeMap.get(lanClassStr).getFontColor() + "\">" + content.getSubtitleLine() + "<br>" + subTitleSb.toString() + "</font>");
							}else{
